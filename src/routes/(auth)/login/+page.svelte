...
<script lang="ts">
    import { login, signInWithGoogle, authError, authLoading } from '$lib/stores/authStore';
...
<div class="bg-zinc-800 p-6 sm:p-8 border border-zinc-700 shadow-xl" in:fade={{ duration: 300 }}>
    ...
    <form on:submit|preventDefault={handleSubmit} class="space-y-5">
        ...
        <div>
            ...
            <div class="relative">
                <input type={showPassword ? "text" : "password"} id="password" name="password" bind:value={password} required
                       class="w-full bg-zinc-700/70 border border-zinc-600 text-gray-200 placeholder-zinc-500 text-sm p-2.5 focus:outline-none focus:ring-2 focus:ring-green-500/70 focus:border-green-500 transition-colors"
                       placeholder="••••••••" />
                <button
                    type="button"
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center">
                <input id="remember-me" name="remember-me" type="checkbox" bind:checked={rememberMe}
                       class="h-4 w-4 text-green-500 bg-zinc-700 border-zinc-600 focus:ring-green-500 focus:ring-offset-zinc-800" />
